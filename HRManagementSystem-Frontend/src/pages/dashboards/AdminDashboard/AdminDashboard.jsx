import React, { useState, useEffect } from 'react';
import { jwtDecode } from 'jwt-decode';
import DashboardLayout from '../../../components/layout/DashboardLayout';
import { Breadcrumb } from '../../../components/layout/Breadcrumb';
import { ChartBarIcon, UsersIcon, ShieldCheckIcon, CogIcon } from '../../../components/ui/Icons';
import CreateHrModal from '../../../components/modals/CreateHrModal';
import { Link } from 'react-router-dom';

const StatCard = ({ title, value, icon, color }) => (
    <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border border-white/30 dark:border-gray-700/30 p-6 rounded-lg shadow-lg hover:-translate-y-1 hover:shadow-2xl transition-all duration-300">
        <div className={`text-3xl ${color} mb-2`}>{icon}</div>
        <h3 className="text-gray-500 dark:text-gray-400 text-sm font-medium uppercase tracking-wider">{title}</h3>
        <p className="text-3xl font-bold text-gray-800 dark:text-white mt-1">{value}</p>
    </div>
);

const AdminDashboard = () => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [currentUser, setCurrentUser] = useState(null);

    useEffect(() => {
        const token = localStorage.getItem('authToken');
        if (token) {
            try {
                setCurrentUser(jwtDecode(token));
            } catch (error) {
                console.error("Invalid token:", error);
            }
        }
    }, []);

    // Simulated user data for the recent activity table
    const recentUsers = [
        { id: 1, name: 'Jane Doe', email: 'jane.d@example.com', role: 'HR Manager', status: 'Active' },
        { id: 2, name: 'John Smith', email: 'john.s@example.com', role: 'Employee', status: 'Active' },
    ];

    return (
        <DashboardLayout
            role="admin"
            title="Admin Dashboard"
            userName={currentUser?.fullName || 'System Admin'}
            userEmail={currentUser?.sub || ''}
        >
            <CreateHrModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />

            <div className="p-8">
                <div className="mb-6">
                    <Breadcrumb items={[{ label: 'Admin Dashboard' }]} />
                </div>

                {/* Top Statistics Cards */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <StatCard title="Total Users" value="1,250" icon={<UsersIcon className="h-8 w-8"/>} color="text-blue-500" />
                    <StatCard title="System Uptime" value="99.98%" icon={<ShieldCheckIcon className="h-8 w-8"/>} color="text-green-500" />
                    <StatCard title="API Requests (24h)" value="1.2M" icon={<ChartBarIcon className="h-8 w-8"/>} color="text-yellow-500" />
                    <StatCard title="Pending Approvals" value="3" icon={<CogIcon className="h-8 w-8"/>} color="text-red-500" />
                </div>

                {/* Main Grid: User Management and System Controls */}
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {/* User Management Panel */}
                    <div className="lg:col-span-2 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border border-white/30 dark:border-gray-700/30 p-6 rounded-lg shadow-lg">
                        <h3 className="text-xl font-semibold text-gray-800 dark:text-white mb-4">Manage Users</h3>
                        <p className="text-gray-600 dark:text-gray-400 mb-4">Create HR accounts or view all users in the system.</p>
                        
                        <div className="mt-4 flex flex-wrap gap-3">
                           <Link 
                                to="/admin/user-management" // Updated link
                                className="bg-gray-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-gray-700 transition-all hover:shadow-lg"
                            >
                                View All Users
                            </Link>
                           <button 
                                onClick={() => setIsModalOpen(true)} 
                                className="bg-green-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-700 transition-all hover:shadow-[0_0_15px_rgba(22,163,74,0.5)]"
                            >
                                Create HR Account
                            </button>
                        </div>
                        
                        <div className="mt-6">
                            <h4 className="font-semibold text-gray-700 dark:text-gray-300 mb-2">Recent User Activity</h4>
                            <table className="w-full text-left bg-transparent">
                                <thead>
                                    <tr className="border-b border-gray-200 dark:border-gray-700">
                                        <th className="p-2 font-semibold text-gray-600 dark:text-gray-400">Name</th>
                                        <th className="p-2 font-semibold text-gray-600 dark:text-gray-400">Email</th>
                                        <th className="p-2 font-semibold text-gray-600 dark:text-gray-400">Role</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {recentUsers.map(user => (
                                        <tr key={user.id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                                            <td className="p-2 text-gray-800 dark:text-gray-200">{user.name}</td>
                                            <td className="p-2 text-gray-600 dark:text-gray-400">{user.email}</td>
                                            <td className="p-2"><span className="bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300 text-xs font-medium px-2 py-1 rounded">{user.role}</span></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        </div>
                    </div>

                    {/* System Controls Panel */}
                    <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border border-white/30 dark:border-gray-700/30 p-6 rounded-lg shadow-lg">
                        <h3 className="text-xl font-semibold text-gray-800 dark:text-white mb-4">System Controls</h3>
                        <ul className="space-y-3">
                            <li><Link to="/admin/system-settings" className="flex items-center text-green-700 dark:text-green-400 hover:underline"><CogIcon className="h-5 w-5 mr-2"/> Manage System Settings</Link></li>
                            <li><Link to="/admin/reports" className="flex items-center text-green-700 dark:text-green-400 hover:underline"><ChartBarIcon className="h-5 w-5 mr-2"/> Generate Reports</Link></li>
                            <li><Link to="/admin/compliance" className="flex items-center text-green-700 dark:text-green-400 hover:underline"><ShieldCheckIcon className="h-5 w-5 mr-2"/> View Compliance Rules</Link></li>
                        </ul>
                    </div>
                </div>
            </div>
        </DashboardLayout>
    );
};

export default AdminDashboard;