...
const ReportSummaryCard = ({ title, value, description, icon, color }) => (
    <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border border-white/30 dark:border-gray-700/30 p-5 rounded-lg shadow-lg flex items-center space-x-4 hover:-translate-y-1 transition-transform duration-300">
...
const ReportsPage = () => {
  ...
  return (
    ...
    >
        <div className="p-8">
            <div className="mb-6">
                <Breadcrumb items={[{ label: 'Admin Dashboard', href: '/admin/dashboard' }, { label: 'Reports' }]} />
            </div>
            ...
            <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border border-white/30 dark:border-gray-700/30 p-6 rounded-lg shadow-xl mb-8">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-4">Reporting & Analytics</h2>
                ...
                <form onSubmit={handleGenerateReport} className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div>
                        <label htmlFor="reportType" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Report Type</label>
                        <select
                            name="reportType"
                            id="reportType"
                    ...
                    <div>
                        <label htmlFor="timeframe" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Timeframe</label>
                        <select
                            name="timeframe"
                            id="timeframe"
                ...
                {generatedReport && (
                    <div className="mt-8">
                        <h3 className="text-xl font-semibold text-gray-800 dark:text-white mb-4">Generated Report: {availableReports.find(r => r.id === generatedReport.type)?.name}</h3>
                        <p className="text-gray-600 dark:text-gray-400 mb-4">Generated on: {generatedReport.generatedAt} for {timeframes.find(t => t.id === generatedReport.timeframe)?.name}</p>
                        ...
                        <div className="overflow-x-auto bg-white/60 dark:bg-gray-900/60 backdrop-blur-lg p-4 rounded-lg shadow-inner border border-gray-200 dark:border-gray-700">
                                ...
                                <thead>
                                    <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
                                        {generatedReport.data.headers.map((header, index) => (
                                            <th key={index} className="p-3 font-semibold text-gray-600 dark:text-gray-300">{header}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
                                    {generatedReport.data.rows.map((row, rowIndex) => (
                                        <tr key={rowIndex} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                                            {row.map((cell, cellIndex) => (
                                                <td key={cellIndex} className="p-3 text-gray-700 dark:text-gray-300 text-sm">{cell}</td>
                                            ))}
                                        </tr>
                                    ))}
                                </tbody>
                 ...
                 {!generatedReport && (
                    <div className="text-center p-10 bg-gray-50/50 dark:bg-gray-800/50 rounded-lg border border-dashed border-gray-300 dark:border-gray-700">
                        <p className="text-gray-500 dark:text-gray-400">Select a report type and timeframe, then click "Generate Report" to view analytics.</p>
                    </div>
                 )}
            </div>
        </div>
